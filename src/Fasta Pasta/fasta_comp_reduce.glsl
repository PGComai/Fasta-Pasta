...
my_data_buffer;
layout(set = 0, binding = 1, std430) restrict buffer MyResultBuffer {
	int data[];
}
my_result_buffer;
...
uint remap(int likeness) {
	return uint(1080.0 * (float(likeness) / 100.0));
}
...
void main() {
	...
	atomicAdd(my_result_buffer.data[location], int(pixel_height));
}
